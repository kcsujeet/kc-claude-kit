...
5. **Defer to built-ins on overlap.** If a rule in the conventions file duplicates a built-in gate (naming, clarity, structure, simplicity, datetime, react, i18n, testing, correctness, verification), that finding is reported under the built-in gate, not here. Don't double-report the same violation under two gates.
